...
class EditUserProfileInfoFragment : Fragment() {
    ...
    private fun observeLiveData() {
        ...
        viewModel.signOutMessage.observe(viewLifecycleOwner, Observer {
            when(it.status){
                Status.SUCCESS->{
                ...
                Status.ERROR->{
                    Toast.makeText(requireContext(), "Bir Hata Oluştu, daha sonra tekrar deneyin", Toast.LENGTH_SHORT).show()
                }
               else->{
                   Toast.makeText(requireContext(), "Çıkış Yapılıyor", Toast.LENGTH_SHORT).show()
               }
            }
